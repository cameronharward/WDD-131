...
function validateForm(event) {
  ...
  const theSelect = e.tartget;
    // Show the container based on the selected payment method, and add the required attribute back.
    if (theSelect.value == "paypal") {
        creditCardContainer.classList.removeAttribute("hide");
        creditCardContainer.classList.setAttribute("required, 1");
    }
  }
  ...
  document.querySelector("#paymentType").addEventListener("change", )
